...
import { FaArrowLeft, FaTag, FaCheck, FaClipboard, FaChevronDown, FaChevronRight } from "react-icons/fa";
import { ShortDivider } from "@/utility/Dividers";
import GoToButton from "@/utility/GoToButton";
import { Dispatch, SetStateAction, useState } from "react";
import { CopyToClipboard } from "react-copy-to-clipboard";
import { motion } from "framer-motion";
import GiscusComments from "@/utility/GiscusComments";

export function ClickToCopyCode({ children }: { children: React.ReactNode }) {
    const [copiedCode, setCopiedCode]: [copiedCode: boolean, setCopiedCode: Dispatch<SetStateAction<boolean>>] =
...
function DropdownTOC({ TOCData }: { TOCData: { level: number; content: string }[] }) {
    ...
    const getLevel2Items = (level1Index: number) => {
        ...
        const endIndex = nextLevel1 ? TOCData.findIndex((item) => item === nextLevel1) : TOCData.length;

        return TOCData.slice(startIndex + 1, endIndex).filter((item) => item.level === 2);
    };
    ...
    const getDirectLevel3Items = (level1Index: number) => {
        ...
        return TOCData.slice(startIndex + 1, endIndex).filter((item) => item.level === 3);
    };

    // Get level 3
    const getLevel3Items = (level2Item: { level: number; content: string }, level1Index: number) => {
    ...
    return (
        <div className="flex flex-col gap-2">
            {level1Items.map((level1, i) => {
                ...
                return (
                    <div key={i} className="border-1 border-gray-600 rounded-3xl backdrop-blur-md sm:p-5 p-3">
                        {/* Heading (LVL 1) Item */}
                                            ...
                                            {openLevel2.has(level2.content) && (
                                                <div className="ml-6 mt-1">
                                                    {getLevel3Items(level2, i).map((level3, k) => (
                                                        ...
                                                        >
                                                            <a href={`#${level3.content}`} className="hover:underline">
                                                                {level3.content}
                                                            </a>
                                                        </div>
                                                    ))}
        ...
        <div className="flex min-[1531px]:px-20 px-0 scroll-mt-28">
            ...
            >
                ...
                <div className="border-1 border-gray-600 rounded-3xl backdrop-blur-md sm:p-5 p-3">
                    ...
                    >
                        <Link href="/blogs" className="text-[#1793D1] hover:text-[#fdf3f3] transition-colors mb-2">
                            <FaArrowLeft />
                        </Link>
                    </motion.div>
                ...
                <div className="min-[1531px]:hidden">
                    {/* Heading */}
                    <h1 className="font-bold text-2xl mx-5 sm:mx-3 mb-2">Table of Content</h1>
                ...
                {/* Go Back button */}
                <GoToButton title={"Back to Blogs"} link="/blogs" animationDelay={1.9} customCSS="mb-10" />
